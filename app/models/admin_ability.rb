  ...
  def initialize(user)
    ...
    if user.is?(:super_admin)
      ...
      if user.is?(:sector_coordinator)
        ...
        can [:read, :update], Teacher, {:zones => {:id => user.accessible_zones(User::ROLE_ACCESS_HIERARCHY[:sector_coordinator][:text]).map(&:id).uniq}}
        can [:read, :update], Teacher, {:secondary_zones => {:id => user.accessible_zones(User::ROLE_ACCESS_HIERARCHY[:sector_coordinator][:text]).map(&:id).uniq}}
